...
program pareto_sampler
    ...
    integer :: n_particles, n_times, io, status, th, nthreads
    real(8) :: intercept, slope, intercept_err, slope_err, mu
    real(8), allocatable :: results(:,:)
    allocate(results(96, 3))  ! 96 iterations, 3 values per iteration
...
CALL OMP_SET_NUM_THREADS(nthreads)
  ...
  !$OMP END PARALLEL
    ...
    !$OMP PARALLEL DO PRIVATE(i, mu, y_series, y_mean, y_sigma, intercept, slope, intercept_err, slope_err, status) &
    !$OMP& SHARED(n_times, nseries, t_array, t_log_array, results) &
    ...
    do j = 5, 100
        ...
        results(j-4, 1) = mu
        results(j-4, 2) = slope
        results(j-4, 3) = slope_err
    end do
    !$OMP END PARALLEL DO

    open(newunit=io, file="slopes.txt")
    do j = 1, 96
        write(io, *) results(j, 1), results(j, 2), results(j, 3)
    end do
    close(io)
