...
contains
    ...
    ! Levy walk simulation with optimized time checking
    function walk(n, mu, t_array) result(mad_array)
        ...
        integer, dimension(n) :: ct  ! Current time index for each walker
        integer :: i, j, step
        ...
        do i = 1, n
            do step = 1, max_steps
                ...
                ! Only check times starting from ct(i) - no need to recheck earlier times
    ...
    subroutine test_walk_monotonicity()
        ...
        do i = 2, n_times
            if (mad_array(i) < mad_array(i-1) - 1.0d-6) then
                is_monotonic = .false.
                print *, mad_array(i), mad_array(i-1)
                exit
            end if
        end do
